...
- 🔴 **时长写的是「表单档位」，不是精确时长**（只狼写 `200-500h`，**不是** `200h+`）——因为表单的时长是**下拉选择**，只能选档位；网站上直接写档位，填表时照抄，不用现场换算
  - 🔴 **用表单里的完整选项文字、带前缀**（2026-09-21 阿聪对照投递表单截图定的）：
    - **单机 → `单机时长 X`**：`0-20h / 20-50h / 50-100h / 100-200h / 200-500h / 500h+`
    - **网游 / 手游 → `网游活跃 X`**：`0-1月 / 1-3月 / 3-6月 / 6-12月 / 1-2年 / 2年+`
    - 例：`单机时长 200-500h`（**不是** `200-500h`）／`网游活跃 2年+`（**不是** `2年及以上`）
  - 21 个游戏的档位对照表在 `2、简历投递/投递管理/投递模板/游戏经历.md`
- 网站**不再显示精确时长**（原来的 `200h+` 已被档位替代）——精确数据如需回溯，看 git 历史或上面的档位对照表
- 🔴 **「游玩程度」的写法 —— 阿聪的格式习惯（2026-09-21 定，照着写）**：

  1. **编号 + 维度名**：`1、主线进度：` `2、收集度：` `3、等级：`
     ⚠️ **只有一条时不要加编号**——直接写 `主线进度：xxx`（阿聪 2026-09-21 定）
  2. **顺序固定**：**主线进度永远第一条** → 收集度 → 等级／成就（有几条写几条，不用凑齐五项）
  3. **每条的结构**：`几个字概括（具体项、具体项）` —— 概括词在前，**括号内的具体项一律用「、」分隔**（不用 `+` `/` `；`）
  4. **换行用 `<br>`**，grid 会自动悬挂对齐在值列，不用额外缩进

```html
<span class="desc-label">游玩程度：</span><span>1、主线进度：全结局通关（断绝不死、复归常人、龙之还乡、修罗）<br>2、收集度：全收集（佛珠、葫芦种子、忍义手、技能树全点满、忍杀忍术）<br>3、等级：满级</span>
```

  维度名参考表单提示语：**等级 / 成就 / 付费 / 收集度 / 主线进度**

- 🔴 **给新游戏补「游玩程度」时，必须连格式草案一起给**（2026-09-21 阿聪要求，**以后加新游戏照此办理**）：

  **不要只罗列收集要素线索**——搜完后**直接按上面的格式习惯，把「游玩程度」的示例段落写出来**，阿聪对着增删即可。

  **已知的三个样板**：

```
只狼
1、主线进度：全结局通关（断绝不死、复归常人、龙之还乡、修罗）
2、收集度：全收集（佛珠、葫芦种子、忍义手、技能树全点满、忍杀忍术）
3、成就：无伤最终 boss

中土世界战争之影
1、主线进度：主线通关（魔多、米那斯伊希尔）
2、收集度：全收集（伊希尔丁之门全开、传送塔全净化、刚铎文物全找回）
3、等级：满级

艾尔登法环
1、主线进度：全结局 + DLC 通关（艾尔登之王、星星时代（菈妮线）、癫火之王、黄金树幽影）
2、等级：100+
```
  ...
  **要点**：
  - 顺序：**主线进度 → 收集度 → 等级／成就**
  - 每条 = **概括词 + 括号**（括号内具体项用「、」）
  - **不用凑齐五项**，有事实的才写；**每条都尽量带括号列具体项**（实在列不出才只写概括）
  - 🔴 **主线进度统一写「全结局通关（<结局1>、<结局2>）」** —— 概括词固定用「**全结局通关**」，**不要**写「一周目通关」这类；括号里列**具体结局名**
    - 例：只狼「全结局通关（断绝不死、复归常人、龙之还乡、修罗）」／黑神话悟空「全结局通关（戴金箍、真结局）」
  - 草案是**给阿聪改的**，他报什么填什么，**不要替他编**（「全收集」「白金」这类硬话必须他确认）
